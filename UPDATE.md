...
### 英语跟读：压缩垂直间距，字都能显示
- 行高压缩：英文 1.1→1.05、中文 1.7→1.35、备注 1.65→1.35
- 段间距压缩：英文下 20→10px、备注上 20→10px
- 大字时内容总高明显降低，更不易被底部控制条遮挡

## v6.2.6

### 英语跟读：仅拓宽内容区宽度（排版不变）
- 回退 v6.2.5 的排版改动（中英并排/vh 字号）
- 内容区宽度 **900px → 1400px**：字大时换行更少、高度更低，
  原竖排布局与字号保持不变

## v6.2.5

### 英语跟读：一屏完整显示，不滚动
- 内容区改为 **flex 撑满 + 垂直居中**（不再依赖滚动查看）
- 字号改为 **vh 自适应**（随屏高缩放），默认一屏完整放下
- **英文与中文左右并排**（宽屏横向利用宽度，窄屏自动换行）——
  大字时也不会垂直超高
- 底部控制条 z-index 提升，悬浮于内容之上

## v6.2.4

### 英语跟读：大字号内容不再被底部控制条盖住
- 内容区底部留位 120px → **200px**（字号控件加入后控制条变高），
  字号调大后滚动到底内容完整可见

## v6.2.3

### 英语跟读：新增字号调节
- 跟读页底部控制条新增**字号 stepper**（− / +，60%~200%，
  步进 10%），实时缩放英文/中文/备注字号
- config 新增 `defaultFontSize`（默认 100%），每次进入按默认值

## v6.2.2

### 顶栏点名结果右对齐
- 结果区改为**右对齐**：短名字靠右显示；长名字（多人）显示末尾，
  开头向左滑动查看

## v6.2.1

### 顶栏点名结果居中显示
- 短名字由 text-align:center 居中（原有）
- **长名字（多人）自动滚动到中间**——视觉居中显示中间部分，
  左右仍可滑动查看全部

## v6.2.0

### UI 焕新：液态玻璃（Glassmorphism）
- **玻璃设计令牌**：--glass-bg / --glass-blur(14px) / --glass-border /
  --glass-shadow，深浅主题分别定义
- **背景光晕**：主站与后台页面加柔和渐变光晕，玻璃模糊有内容可透
- **玻璃化元素**（blur）：
  - 主站/后台顶栏（sticky 固定，滚动内容从玻璃下穿过）
  - 点名面板、更新日志/关于/系统日志弹窗、更多菜单、下拉面板
  - 后台面板卡片、登录卡片
- **半透明元素**（不 blur，性能安全）：出题卡片
- **性能**：backdrop-filter 仅用于常驻少量元素，点名动画层无 blur；
  不支持 backdrop-filter 的浏览器自动降级为实色（@supports）

## v6.1.42

### 顶栏结果区：支持鼠标拖拽滑动
- 除滚轮横滚外，新增**鼠标按住拖动**滚动（cursor 已显示 grab）
- 结果区可滚动时按下拖动即可左右查看

## v6.1.41

### 多人卡片：缩短展示态停顿
- 弹出后到展开签名（FLIP）的停顿 1800ms → **1200ms**，节奏更紧凑

## v6.1.40

### 修复：多人时随机过程动画只展示一个人
- 时间加权随机法（fair）最终阶段改为**从候选池无重复抽 N 人**
  （优先未点名者，不足逐级补抽），过程动画的"选定结果"显示
  **选中的全部 N 人**（自适应字号）
- 单人行为不变（等价于抽 1 人）
...
### 修复：多人 + 显示随机过程的动画 bug
- 根因：过程动画的结果区把所有名字拼在 2.5rem 大字一行，
  多人（最多 20 人）时严重溢出/换行混乱
- 修复：结果区**按人数自适应字号**（2.5→1.2rem）+ 限宽 84vw
  + 自动换行，并显示人数（"结果（N 人）"）

## v6.1.38

### 顶栏结果区：滚轮横滚 + 禁选中 + 更宽靠左
- **滚轮水平滚动**：结果区可滚动时，滚轮/触控板直接横向滑动
- **禁止选中文字**（user-select: none），滑动不拖蓝
- **更宽更靠左**：结果区占满品牌与右侧按钮之间全部空间
  （flex:1），名字多时从品牌后开始延伸，接近最左边

## v6.1.37

### 顶栏点名结果：限宽 + 左右滑动
- 结果位限宽 **38vw**，多人名字多时不再挤占/换行顶栏其他元素
- 超出部分**左右滑动**查看（触摸/触控板/Shift+滚轮），
  **无可见滚动条**（scrollbar 隐藏）

## v6.1.36

### 收尾动画高级化（丝滑多层）
- 内容：**上浮 + 轻微放大（1.03）+ 模糊消散**（0.55s easeOutQuint）
- 背景稍慢淡出（0.55s）营造层次
- 顶栏结果**淡入**（0.28s）而非瞬间显示
- 三层动画时序呼应，过渡丝滑有质感
...
### 点击后的收尾动画彻底简化
- 放弃"名字飞行"方案（多次迭代仍不自然）
- 改为：内容（多人卡片/单卡）**整体向上轻滑 + 淡出**（0.35s），
  背景同步淡出，顶栏显示点名结果——干净自然，无交叉无跳变

## v6.1.34

### 开屏动画精简
- 启动自检日志不再铺满开屏（完整日志保留在「系统日志」弹窗）
- 开屏仅在**有问题时**显示一行错误摘要（"⚠ 发现 N 个问题"），
  正常时保持 logo + 标题 + 版本 + 提示的简洁视觉
- 自检立即完成（间隔 20ms→0），点击开屏无需等待
- 顺带修复：自检中"SeaScribe 版本"硬编码 v5.0.0 → 读取配置版本号

## v6.1.33

### 多人飞入：一起飞 + 飞到顶栏各自位置
- 所有名字 **0 延迟同时飞入**（不再逐个交错）
- 每个名字飞到顶栏**各自的位置**（顶栏按名字拆分 span 测量目标），
  不再全部汇聚到同一个点
- 0.5s 同步过渡 + 卡片整体淡出
...
### 多人飞入重构：ghost 副本方案
- 名字复制为 **ghost 副本**（fixed 定位在原卡片位置，脱离卡片容器）
  依次飞到顶栏中心淡出；卡片整体独立淡出——两者互不影响，
  不再有裁剪/遮盖/布局冲突
- cleanup 不再恢复单卡布局（交给 shrink 收尾），避免点击瞬间
  装饰层出现空单卡与多人卡片叠加的错乱

## v6.1.31

### 修复：多人飞入动画三处 bug
- **卡片被提前清空**：waitForClick 的 cleanup 清空了卡片容器，
  飞入拿不到名字元素 → 保留卡片给 shrink 收尾清理
- **名字飞入被盖住**：卡片容器立即 opacity:0 使名字跟随消失 →
  改为头像/签名先淡出、名字保留飞入、容器延迟淡出
- **名字被卡片裁剪**：卡片 overflow:hidden 裁剪向上飞出的名字 →
  飞入前临时放开 overflow:visible

## v6.1.30

### 多人飞入：名字从各自卡片原位向上飞入
- 不再"收起卡片 + 中心拼一行再飞"
- 每个名字**从卡片内的原位置**向上飞到顶栏高度并淡出
  （逐个 40ms 交错，有层次感），卡片同步淡出，顶栏显示结果
- 单人逻辑不变（淡入 + 纯平移）

## v6.1.29

### 修复：飞入动画名字先变大
- 根因：4vw 字号在小屏小于顶栏字号 → scale>1，动画中名字变大
- 修复：飞入字号**直接对齐顶栏目标字号**（scale≈1），
  动画为纯平移——名字从中心平滑移到顶栏，大小不变

## v6.1.28

### 修复：飞入顶栏时名字过大
- 飞入字号从 12vw/6vw 大幅降至 **4vw（单人）/3vw（多人）**，
  名字以接近顶栏的中小字号出现并飞入，不再闪现大名字
- 淡入缩短至 0.12s，节奏更紧凑

## v6.1.27

### 修复：点击后名字"突然变大然后上移"
- 根因：卡片收起瞬间，满尺寸名字无过渡直接出现（多人时从紧凑
  小名字跳到中心大名字），再开始飞入——视觉突兀
- 修复：飞入前名字先**柔和淡入**（0.18s opacity 过渡），
  淡入完成后再测量并播放缩小飞入动画

## v6.1.26

### 修复：点击屏幕后名字飞入顶栏动画视觉 bug
- 多人：卡片收起时恢复单卡布局，但**清空头像/签名/时间/提示**
  残留（避免空元素闪烁），只留名字飞入
- 单人：同样清空 fill 填充的头像/签名，飞入动画只剩名字，干净利落
- 测量前强制重排（offsetWidth），dx/dy/scale 计算基于稳定布局

## v6.1.25

### 多人点名：人数多时排版自适应
- **按人数动态列数**：1-2 人 1 列、3-4 人 2 列、5-7 人 3 列、
  8-11 人 4 列、12-17 人 5 列、18-20 人 6 列（窄屏自动降列）
- 卡片高度降低（clamp 96-150px），展示态头像/名字、紧凑态元素
  等比缩小适配矮卡
- 多人卡片容器限高 86vh，人数极多时内部滚动，不溢出屏幕

## v6.1.24

### 多人点名人数上限 10 → 20
- 面板人数可调范围扩至 **1-20**，提示文案同步更新
