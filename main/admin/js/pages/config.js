...
function parseConfig(raw) {
  ...
  // 间接引用格式: window.__XXX_CONFIG__ = window._CONF.xxx
  // 回退到文件中搜索真正的对象定义 window._CONF.xxx = { ... }
  var indRef = after.match(/^window\._CONF\.(\w+)$/);
  if (indRef) {
    var key = indRef[1];
    var realMatch = raw.match(new RegExp('window\\._CONF\\.' + key + '\\s*=\\s*\\{'));
    if (realMatch) {
      // 从 { 开始匹配到对应的 }
      var braceStart = raw.indexOf('{', raw.indexOf(realMatch[0]));
      var depth = 0, braceEnd = braceStart;
      for (var j = braceStart; j < raw.length; j++) {
        if (raw[j] === '{') depth++;
        else if (raw[j] === '}') { depth--; if (depth === 0) { braceEnd = j; break; } }
      }
      after = raw.substring(braceStart + 1, braceEnd).trim();
      if (after) after = '{\n' + after + '\n}';
    }
  }
...
function rebuildJS(originalRaw, formData) {
  ...
  // 间接引用格式: window.__XXX_CONFIG__ = window._CONF.xxx
  // 替换 window._CONF.xxx = { ... } 对象体，保持其余内容不变
  var indM = originalRaw.match(/window\.__\w+_CONFIG__\s*=\s*window\._CONF\.(\w+)\s*;?\s*$/m);
  if (indM) {
    var key = indM[1];
    var re = new RegExp('(window\\._CONF\\.' + key + '\\s*=\\s*)\\{[\\s\\S]*?\\n\\};');
    var replaced = originalRaw.replace(re, '$1' + body + '\n};');
    if (replaced !== originalRaw) return replaced;
    // fallback: body might end with newline already
    replaced = originalRaw.replace(re, '$1' + body.replace(/\n$/, '') + '\n};');
    return replaced;
  }

  // 直接赋值格式（旧版）: window.__XXX_CONFIG__ = { ... };
  var dm = originalRaw.match(/^(.*?=\s*)\{[\s\S]*\}(\s*;?\s*)$/);
  if (!dm) return JSON.stringify(formData, null, 2);
  return dm[1] + body + dm[2];
}
